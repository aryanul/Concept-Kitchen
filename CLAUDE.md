...
`JWT_ACCESS_SECRET`. The DB connection reads `DATABASE_URL` if set, otherwise the discrete
`DB_HOST`/`DB_PORT`/`DB_USER`/`DB_PASSWORD`/`DB_NAME` fields. The web app reads
`VITE_API_URL` (in `web/.env.local`, points at `…/api/v1`).
...
- **IDs are ULIDs** (`ulid()`), not auto-increment. Human-facing codes (e.g. `BR001`) are
  generated by the `nextCode()` helper in `masters.ts`.
