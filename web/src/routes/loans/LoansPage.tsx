...
export function LoansPage() {
  ...
  useEffect(() => {
    api.get<Resp>('/loans')
      .then((r) => { setLoans(r.data.data); setTotal(r.data.meta.total); setStats(r.data.stats); })
      .catch(() => {})
  ...
  return (
    <div>
      <PageHeader title="Advances & Loans" subtitle="Employee loan and advance disbursements."
        actions={<Button icon={Plus} variant="primary">New Loan</Button>} />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--ck-bg)', textAlign: 'left' }}>
                {['Employee', 'Kind', 'Principal', 'Outstanding', 'EMI', 'Tenure', 'Remaining', 'Status'].map((h) => (
            ...
            <tbody>
              ...
              {loans.map((l, i) => (
                <tr key={l.id} style={{ borderTop: '1px solid var(--ck-line)' }}
                  ...
                  <td style={{ padding: '12px 16px' }}>{l.tenure_months} mo</td>
